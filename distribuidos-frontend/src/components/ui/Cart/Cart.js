...
export default function Cart({balance, userId}) {
    ...
    const total = cartItems.reduce((previus, current) => previus + current.cantidad * current.precio, 0);
    ...
    return(
        <div className="cart-container">
            <div className="cart-icon-container">
                ...
                {!cartOpen && (
                    <div className="number-items"><p>{productsLength}</p></div>
                )}
            </div>
            ...
            <div className={`data-cart-container ${!cartOpen && "no-visible"}`}>
                {cartItems && cartOpen && (
                    <>
                        ...
                        <button className="btn-pagar" onClick={handlePago}>Pagar</button>
